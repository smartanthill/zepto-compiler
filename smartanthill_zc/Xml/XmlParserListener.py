# Generated from java-escape by ANTLR 4.5
from antlr4 import *

# This class defines a complete listener for a parse tree produced by XmlParser.
class XmlParserListener(ParseTreeListener):

    # Enter a parse tree produced by XmlParser#document.
    def enterDocument(self, ctx):
        pass

    # Exit a parse tree produced by XmlParser#document.
    def exitDocument(self, ctx):
        pass


    # Enter a parse tree produced by XmlParser#prolog.
    def enterProlog(self, ctx):
        pass

    # Exit a parse tree produced by XmlParser#prolog.
    def exitProlog(self, ctx):
        pass


    # Enter a parse tree produced by XmlParser#xmlDecl.
    def enterXmlDecl(self, ctx):
        pass

    # Exit a parse tree produced by XmlParser#xmlDecl.
    def exitXmlDecl(self, ctx):
        pass


    # Enter a parse tree produced by XmlParser#emptyTagRule.
    def enterEmptyTagRule(self, ctx):
        pass

    # Exit a parse tree produced by XmlParser#emptyTagRule.
    def exitEmptyTagRule(self, ctx):
        pass


    # Enter a parse tree produced by XmlParser#seTagRule.
    def enterSeTagRule(self, ctx):
        pass

    # Exit a parse tree produced by XmlParser#seTagRule.
    def exitSeTagRule(self, ctx):
        pass


    # Enter a parse tree produced by XmlParser#emptyElemTag.
    def enterEmptyElemTag(self, ctx):
        pass

    # Exit a parse tree produced by XmlParser#emptyElemTag.
    def exitEmptyElemTag(self, ctx):
        pass


    # Enter a parse tree produced by XmlParser#sTag.
    def enterSTag(self, ctx):
        pass

    # Exit a parse tree produced by XmlParser#sTag.
    def exitSTag(self, ctx):
        pass


    # Enter a parse tree produced by XmlParser#attribute.
    def enterAttribute(self, ctx):
        pass

    # Exit a parse tree produced by XmlParser#attribute.
    def exitAttribute(self, ctx):
        pass


    # Enter a parse tree produced by XmlParser#eTag.
    def enterETag(self, ctx):
        pass

    # Exit a parse tree produced by XmlParser#eTag.
    def exitETag(self, ctx):
        pass


    # Enter a parse tree produced by XmlParser#content.
    def enterContent(self, ctx):
        pass

    # Exit a parse tree produced by XmlParser#content.
    def exitContent(self, ctx):
        pass


